            ...
            realData.alertas_reposicion.slice(0, 2).forEach(item => {
                accionesHtml += `
                    <div class="bg-navy-premium border border-red-900/30 p-4 rounded-xl relative overflow-hidden group">
                        ...
                        <button class="w-full bg-red-900/40 hover:bg-red-700/60 border border-red-500/50 text-white text-xs py-1.5 rounded-lg transition-colors shadow-sm" onclick="window.generarOrdenCompra('${item.id || ''}', decodeURIComponent('${encodeURIComponent(item.producto)}'), ${item.stock || 0}, ${item.stock_minimo || 50}, '${item.telefono_proveedor || ''}', '${item.email_proveedor || ''}')">Generar orden de compra</button>
                    </div>`;
            });
        }
...
function setupGlobalEvents() {
    ...
    // Alternar visibilidad de contraseña en el Login (Hover)
    document.addEventListener('mouseover', (e) => {
        const toggleBtn = e.target.closest('#btn-toggle-password');
        if (toggleBtn) {
            const passwordInput = document.getElementById('login-password');
            const iconContainer = document.getElementById('password-toggle-icon');
    ...
    document.addEventListener('mouseout', (e) => {
        const toggleBtn = e.target.closest('#btn-toggle-password');
        if (toggleBtn) {
            // Evitar flickering si el mouse se mueve entre los SVGs internos
            if (!e.relatedTarget || !toggleBtn.contains(e.relatedTarget)) {
                const passwordInput = document.getElementById('login-password');
                const iconContainer = document.getElementById('password-toggle-icon');
