...
export const renderInventoryRow = (product, userRole) => {
    ...
    <tr class="border-b border-industrial-gray/60 hover:bg-white/5 transition-colors group">
        <td class="px-6 py-4 hidden md:table-cell font-mono text-[11px] text-slate-400 tracking-wider">${product.codigo_skv}</td>
        <td class="px-6 py-4">
            <div class="flex items-center gap-3">
                ...
                <span class="font-bold uppercase text-xs text-white tracking-wide leading-tight">${product.nombre}</span>
            </div>
        </td>
        ...
        <td class="px-6 py-4 font-mono text-xs text-gold font-bold tracking-wide">$ ${formatCurrency(product.precio_usd)}</td>
        <td class="px-6 py-4 font-mono text-xs">
            <div class="flex items-center gap-2">
                ...
                <span class="font-bold ${isLowStock ? 'text-red-400' : 'text-white'}">${formatNumber(product.stock)}</span>
            </div>
        </td>
