...
export const renderNavbar = (storeName, tasa, user = null, userRole = '') => {
    ...
    return `
    <header class="flex flex-col md:flex-row justify-between items-center w-full px-3 md:px-6 py-3 md:py-4 bg-dark-gray border-b border-industrial-gray z-30 relative min-w-0 gap-y-3 md:gap-x-6">
        <div class="flex items-center justify-between md:justify-start gap-1.5 md:gap-4 min-w-0 w-full md:w-auto flex-shrink-0">
            ...
            <!-- Botón de Carrito Móvil visible a la derecha del Logo en celular -->
            <button id="btn-mobile-cart-header" class="relative flex items-center justify-center w-8 h-8 bg-navy border border-industrial-gray text-slate-400 hover:text-gold rounded-full md:hidden flex-shrink-0">
                <span class="material-symbols-outlined text-sm">shopping_cart</span>
                <span id="mobile-cart-badge-header" class="absolute -top-1 -right-1 bg-gold text-navy text-[8px] font-black w-3.5 h-3.5 rounded-full flex items-center justify-center shadow-md hidden">0</span>
            </button>
        </div>
        
        <!-- Barra de Búsqueda Adaptativa -->
        <div id="search-container" class="w-full md:w-auto md:flex-1 max-w-none md:max-w-xl mx-0 md:mx-8 bg-transparent z-20">
            <div class="relative group">
                ...
                <input id="search-input" class="w-full bg-white border border-industrial-gray focus:border-gold focus:ring-1 focus:ring-gold rounded-sm pl-8 md:pl-10 pr-3 md:pr-4 py-1.5 md:py-2 text-black font-bold text-xs uppercase tracking-wide transition-all placeholder-slate-500 outline-none" placeholder="BUSCAR PRODUCTO O SKU..." type="text"/>
            </div>
        </div>
        
        <div class="flex items-center gap-1.5 md:gap-4 flex-shrink-0 w-full md:w-auto justify-end">
            ...
            <!-- Botón de Carrito Móvil en Menú Lateral (Oculto en Escritorio) -->
            ...
            <!-- Contenedor Alineado a la Derecha para Tasa y Usuario -->
            <div class="flex items-center justify-end ml-auto gap-4 flex-shrink-0">
                <div class="text-right hidden md:block">
